...
Oneline=root.Oneline=window.Oneline||{};
 min=Math.min,max=Math.max,pow=Math.pow,sqrt=Math.sqrt,log=Math.log,floor=Math.floor; slice=Array.slice,splice=Array.splice,every=Array.every;websocket={};navigator=window.navigator?window.navigator:[];window.BSON=window.BSON||bson().BSON;BSON.stringify=function(packet){return O.uint8ToString(BSON.serialize(packet));};BSON.parse=function(message){return BSON.deserialize(O.stringToUint8(message));};Oneline.interop=BSON;Oneline.socket=Oneline.socket||[];Oneline.settings=Oneline.settings||{};Oneline.freq=Oneline.freq=Oneline.freq||1000;Oneline.host=Oneline.host||'localhost';Oneline.port=Oneline.port||9000;Oneline.protoline=Oneline.protoline||[];Oneline.connection_uuid="";Oneline.lastResponse="";Oneline.objects=Oneline.objects||[];Oneline.loaded=0;Oneline.setup=function(options){Oneline.settings=options;Oneline.port=options.port||Oneline.port;Oneline.host=options.host||Oneline.host;Oneline.type=options.type==='bind'?'bind':'auto';Oneline.on=options.on||'click';Oneline.usessl=options.usessl||false;Oneline.connection_uuid=Oneline.uuid();Oneline.target=options.target;Oneline.protocol=Oneline.usessl?"wss://":"ws://";options.server=options.module||options.server;if(options.server.match(/ws\:\/\//))
Oneline.settings.server=options.server;else
Oneline.settings.server=options.server=Oneline.protocol+Oneline.host+':'+Oneline.port+'/'+options.server;if(OnelineTransport.WebSockets.detect()){Oneline.socket=OnelineTransport.WebSockets.Ctor(Oneline.settings.server);}else{
...
Oneline.interop=typeof options.interop!=='undefined'?options.interop==='json'?JSON:BSON:BSON;Oneline.freq=typeof options.freq!=='undefined'?options.freq:Oneline.freq;Oneline.socket.onopen=function(){Oneline.loaded=1;};Oneline.socket.onmessage=function(evt){console.log(O.interop.parse(evt.data));};if(typeof options.order!=='undefined'){Oneline.order=options.order;}else{Oneline.order=[];}
Oneline.target=document.getElementById(Oneline.target);};Oneline.contextOptions={"geolocation":{"limit":{"default":512},"range":{"default":40.00},"bidirectional":{"default":false}}};Oneline.fetchOptional=function(mod,option,value){if(typeof value==='undefined'){return Oneline.contextOptions[mod][option]['default'];}
return option;};Oneline.isMe=function(message){if(message.connection_uuid===O.connection_uuid){return true;}
return false;};Oneline.agent=function(options){};Oneline.nodes=function(options){};Oneline.stream=function(options){options=options||{};options.pipeline=options.pipeline||O.protoline;return O.pipeline({},options.pipeline,function(res){console.log(res);}).run();};Oneline.value=function(op,val){return{'op':op,'value':val};}; Oneline.moment=function(start,end){return{'start':0000,'end':3600};}; Oneline.echo=Oneline.echo=function(options,ready){Oneline.echo.options=options;if(!ready)
Oneline.objects.push(clone(Oneline.echo(options,1)));return{ run:function(m){this.m=m||{};this.m.echo={};this.m.echo.limit=O.echo.options.limit;this.state=1;}}};Oneline.geolocation=Oneline.geo=function(options,ready){Oneline.geolocation.options=options;if(!ready)
Oneline.objects.push(clone(Oneline.geo(options,1)));return{ run:function(m)
{this.m=m||{};this.m.geo={};this.state=0;this.m.geo.every=O.geolocation.options.every;this.m.geo.range=O.geolocation.options.range;this.m.geo.limit=O.fetchOptional("geolocation","limit",O.geolocation.options.limit);var that=this;navigator.geolocation.getCurrentPosition(function(res){that.m.geo.lat=res.coords.longitude;that.m.geo.lng=res.coords.latitude;that.m.geo.range=O.geolocation.options.range;that.m.geo.limit=O.geolocation.options.limit;that.state=1;});}};};Oneline.event=function(options,ready){Oneline.event.options=options;if(!ready)
Oneline.objects.push(clone(Oneline.event(options,1)));return{ run:function(m)
{this.m=m||{};this.m.event=Oneline.event.options;this.state=1;}}};Oneline.generic=function(options,ready){Oneline.generic.options=options;if(!ready)
Oneline.objects.push(clone(Oneline.generic(options,1)));return{ run:function(m)
{this.m=m||{};this.m.generic={};this.m.generic.type=Oneline.generic.options.type;this.m.generic.data=Oneline.generic.options.data;this.state=1;console.log(this);}}};Oneline.time=function(options){Oneline.time.options=options;if(!ready)
Oneline.objects.push(clone(Oneline.time(options,1)));return{run:function(m)
{this.m=m||{};this.m.time={};this.m.time.start=Oneline.time.options.moment.start;this.m.time.end=Oneline.time.options.moment.end;this.state=1;}};};Oneline.random=function(options,ready){Oneline.random.options=options;if(!ready)
Oneline.objects.push(clone(Oneline.random(options,1)));return{run:function(m)
...
{O.t=window['set'+O.provider](function(){if(Oneline.running)
return;if(!Oneline.loaded)
return O.connector.connect();if(Oneline.signalStop||O.socket.readyState===2||O.socket.readyState===3){return O.connector.disconnect();}
O.running=1;O.socket.onmessage=function(evt){return O.callback(O.interop.parse(evt.data));};var m={},c=0,m_={},ii=0,t,i=0;for(var i in O.objects){O.objects[parseInt(i)].run(m);O.oot=window['set'+O.runner](function(){if(c!==0&&O.objects[c-1].state!==1)
return;if(typeof O.objects[c]!=='object')
return;if(O.objects[c].state===1){m=collect(m,O.objects[c].m);c++;window['clear'+O.runner](O.oot);}},1);}
O.ooot=window['set'+O.runner](function(){if(c===O.objects.length){m_.packet=m;t=new Date().getTime();m_.packet.order=O.order;m_.packet.timestamp=t;m_.packet.interop=O.interop;m_.uuid=O.uuid();m_.connection_uuid=O.connection_uuid;m_.timestamp=new Date().getTime();O.socket.send(O.interop.stringify(m_));if(typeof O.callback==='function')
O.callback(m_);O.running=0;window['clear'+O.runner](O.ooot);}},1);},O.freq);}};};Oneline.sound=function(options){Oneline.sound.options=options;return{run:function(m)
...
{O.loaded=0;if(O.socket.readyState===3)
