...
if(!needsMerge){Oneline.objects.push(obj);}}; Oneline.once=function(event_type,message){var m_={};var packet={};if(typeof event_type==="object"){packet[event_type.obj]={};packet[event_type.obj]['type']=event_type.data.type;packet[event_type.obj]['data']=event_type.data.data;}else{message['type']=event_type;message['data']={}; for(var i in Object.keys(message)){message['data'][message[i]]=message[i];delete message[i];}}
...
Oneline.target=document.getElementById(Oneline.target);
 ...
 Oneline.connector.connect();};Oneline.contextOptions={"geolocation":{"lat_field":{"default":"lat"},"lng_field":{"default":"lng"},"every":{"default":10.00},"limit":{"default":512},"range":{"default":40.00},"bidirectional":{"default":false}},"time":{"sfield":{"default":"stime"},"efield":{"default":"etime"}}};Oneline.fetchOptional=function(mod,option,value){if(typeof value==='undefined'){return Oneline.contextOptions[mod][option]['default'];}
...
return{ run:function(m){this.m=m||{};this.m.echo={};this.m.echo.limit=O.echo.options.limit;this.state=1;}}};Oneline.geolocation=Oneline.geo=function(options,ready){Oneline.geolocation.options=options;if(!ready){var obj=clone(Oneline.geolocation(options,1));obj.type="geolocation";obj.signature=Oneline.signature;obj.options=Oneline.geolocation.options;Oneline.joinOrNew(obj);}
return{ run:function(m)
{if(this.state!=1){this.m=m||{};this.m.geo={};this.m.geo.every=O.fetchOptional("geolocation","every",O.geolocation.options.every);this.m.geo.range=O.fetchOptional("geolocation","range",O.geolocation.options.range);this.m.geo.limit=O.fetchOptional("geolocation","limit",O.geolocation.options.limit);this.m.geo.lat_field=O.fetchOptional("geolocation","lat_field",O.geolocation.options.lat_field);this.m.geo.lng_field=O.fetchOptional("geolocation","lng_field",O.geolocation.options.lng_field);var that=this;navigator.geolocation.getCurrentPosition(function(res){if(res.coords&&typeof res.coords!=='undefined'){that.m.geo.lat=res.coords.latitude;that.m.geo.lng=res.coords.longitude;that.state=1;}});}}};};Oneline.event=function(options,ready){Oneline.event.options=options;if(!ready){var obj=clone(Oneline.event(options,1));obj.type="event";obj.signature=Oneline.signature;obj.options=Oneline.event.options;Oneline.joinOrNew(obj);}
return{ run:function(m)
...
O.running=1;var m={},c=0,m_={},ii=0,t,i=0;for(var i in O.objects){O.objects[parseInt(i)].run(m);O.oot=window['set'+O.runner](function(){if(O.objects.length>0){if(c!==0&&O.objects[c-1].state!==1)
...
O.running=0;window['clear'+O.runner](O.ooot);}},1);},O.freq);}};};Oneline.sound=function(options,ready){Oneline.sound.options=options;if(!ready){var obj=clone(Oneline.sound(options,1));obj.signature=Oneline.signature;obj.type="sound";obj.options=Oneline.sound.options;Oneline.joinOrNew(obj);}
return{run:function(m)
