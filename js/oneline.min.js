...
if(!needsMerge){Oneline.objects.push(obj);}}; Oneline.once=function(event_type,message){
 var m={};var innerMessage={};var messageKeys=Object.keys(message);innerMessage['type']=event_type;innerMessage['data']={};for(var i in messageKeys){innerMessage['data'][messageKeys[i]]=message[messageKeys[i]];}
m.timestamp=new Date().getTime();m.connection_uuid=O.connection_uuid;m.uuid=O.uuid();m.packet={};m.packet.generic=innerMessage;Oneline.socket.send(Oneline.interop.stringify(m));};Oneline.alwaysConnect=function(){ Oneline.alwaysConnectInterval=setInterval(function(){if(Oneline.socket.readyState===2||Oneline.socket.readyState===3){Oneline.connector.connect();}else{}},0);};Oneline.ready=function(callback){var readyInterval=setInterval(function(){if(Oneline.socket.readyState===1){callback();clearInterval(readyInterval);}else{
...
O.ooot=window['set'+O.runner](function(){if(c===O.objects.length&&O.objects.length>0){m_.packet=m;t=new Date().getTime();m_.timestamp=t;m_.uuid=O.uuid();m_.connection_uuid=O.connection_uuid;if(t>Oneline.after){O.socket.send(O.interop.stringify(m_));}
