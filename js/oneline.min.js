...
return packet;};Oneline.joinOrNew=function(obj){var needsMerge=0;Oneline.signature=Oneline.uid();obj['options']=Oneline[obj.type]['options'];for(var i in Oneline.objects){if(Oneline.objects[i].type===obj.type){delete Oneline.objects[i];Oneline.objects[i]=obj;Oneline.objects[i].state=0;needsMerge=1;}}
if(!needsMerge){Oneline.objects.push(obj);}};Oneline.compareOptions=function(obj1,obj2){var diffCount=0;var keys1=Object.keys(obj1);var keys2=Object.keys(obj2);if(keys1.length!==keys2.length){diffCount++;}
for(var i in keys1){var foundKey=0;var foundVal=0;for(var j in keys2){if(keys2[j]===keys1[i]){foundKey=1;}
if(obj1[keys1[i]]===obj2[keys2[j]]){foundVal=1;}}
if(!foundVal){diffCount++;}
if(!foundKey){diffCount++;}}
return diffCount;};Oneline.updateAllSignatures=function(){for(var i in Oneline.objects){if(Oneline.compareOptions(Oneline.objects[i],Oneline[Oneline.objects[i].type])>0){Oneline.objects[i].signature=Oneline.signature;}}}; Oneline.once=function(event_type,message){var m_={};var packet={};if(typeof event_type==="object"){packet[event_type.obj]={};packet[event_type.obj]['type']=event_type.data.type;packet[event_type.obj]['data']=event_type.data.data;}else{message['type']=event_type; for(var i in Object.keys(message)){message['data'][message[i]]=message;delete message[i];}}
...
Oneline.target=document.getElementById(Oneline.target);
 ...
 Oneline.connector.connect();};Oneline.contextOptions={"geolocation":{"every":{"default":10.00},"limit":{"default":512},"range":{"default":40.00},"bidirectional":{"default":false}}};Oneline.fetchOptional=function(mod,option,value){if(typeof value==='undefined'){return Oneline.contextOptions[mod][option]['default'];}
...
return{ run:function(m){this.m=m||{};this.m.echo={};this.m.echo.limit=O.echo.options.limit;this.state=1;}}};Oneline.geolocation=Oneline.geo=function(options,ready){Oneline.geolocation.options=options;if(!ready){var obj=clone(Oneline.geolocation(options,1));obj.type="geolocation";obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{ run:function(m)
{if(this.state!=1){this.m=m||{};this.m.geo={};this.m.geo.every=O.fetchOptional("geolocation","every",O.geolocation.options.every);this.m.geo.range=O.fetchOptional("geolocation","range",O.geolocation.options.range);this.m.geo.limit=O.fetchOptional("geolocation","limit",O.geolocation.options.limit);var that=this;navigator.geolocation.getCurrentPosition(function(res){if(res.coords&&typeof res.coords!=='undefined'){that.m.geo.lat=res.coords.latitude;that.m.geo.lng=res.coords.longitude;that.state=1;}});}}};};Oneline.event=function(options,ready){Oneline.event.options=options;if(!ready){var obj=clone(Oneline.event(options,1));obj.type="event";obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{ run:function(m)
{this.m=m||{};this.m.event=Oneline.event.options;this.state=1;}}};Oneline.generic=function(options,ready){Oneline.generic.options=options;if(!ready){var obj=clone(Oneline.generic(options,1));obj.type="generic";obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{ run:function(m)
{this.m=m||{};this.m.generic={};this.m.generic.type=Oneline.generic.options.type;this.m.generic.data=Oneline.generic.options.data;this.state=1;}}};Oneline.time=function(options,ready){Oneline.time.options=options;if(!ready){var obj=clone(Oneline.time(options,1));obj.type="time";obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{run:function(m)
{this.m=m||{};this.m.time={};this.m.time.start=Oneline.time.options.moment.start;this.m.time.end=Oneline.time.options.moment.end;this.state=1;}};};Oneline.random=function(options,ready){Oneline.random.options=options;if(!ready){var obj=clone(Oneline.random(options,1));obj.type="random";obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{run:function(m)
...
{Oneline.signalStop=false;Oneline.update=0;Oneline.connector.connect();O.t=window['set'+O.provider](function(){if(Oneline.update){O.connector.clear();return O.pipeline(O.agent,O.objects,O.callback).run();}
if(Oneline.running){return;}
if(Oneline.signalStop||O.socket.readyState===2||O.socket.readyState===3){return O.connector.disconnect();}
O.running=1;var m={},c=0,m_={},ii=0,t,i=0;if(O.objects.length>1){for(var i in O.objects){O.objects[parseInt(i)].run(m);O.oot=window['set'+O.runner](function(){if(c!==0&&O.objects[c-1].state!==1)
return;if(typeof O.objects[c]!=='object')
return;if(O.objects[c].state===1){m=collect(m,O.objects[c].m);c++;window['clear'+O.runner](O.oot);}},1);}}else{if(typeof O.objects[0]!=='undefined'&&O.objects[0].state===1){m=collect(m,O.objects[0].m);c++;window['clear'+O.runner](O.oot);}}
...
O.running=0;window['clear'+O.runner](O.ooot);}},1);},O.freq);}};};Oneline.sound=function(options,ready){Oneline.sound.options=options;if(!ready){var obj=clone(Oneline.sound(options,1));obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{run:function(m)
