...
Oneline=root.Oneline=window.Oneline||{};
 min=Math.min,max=Math.max,pow=Math.pow,sqrt=Math.sqrt,log=Math.log,floor=Math.floor; slice=Array.slice,splice=Array.splice,every=Array.every;websocket={};navigator=window.navigator?window.navigator:[];window.BSON=window.BSON||bson().BSON;BSON.stringify=function(packet){return O.uint8ToString(BSON.serialize(packet));};BSON.parse=function(message){return BSON.deserialize(O.stringToUint8(message));};Oneline.interop=BSON;Oneline.socket=Oneline.socket||[];Oneline.settings=Oneline.settings||{};Oneline.freq=Oneline.freq=Oneline.freq||1000;Oneline.host=Oneline.host||'localhost';Oneline.port=Oneline.port||9000;Oneline.protoline=Oneline.protoline||[];Oneline.objects=Oneline.objects||[];Oneline.after=Date.now();Oneline.signature=""; Oneline.asyncs={}; Oneline.asyncCallbacks={};Oneline.readyIntervalFreq=100; Oneline.acceptAfter=function(time){Oneline.after=time;clearInterval(Oneline.t);clearTimeout(Oneline.t);clearInterval(Oneline.oot);clearTimeout(Oneline.oot);clearInterval(Oneline.ooot);clearTimeout(Oneline.ooot);};Oneline.setAsync=function(key,data,fn){var currentAsyncs=Oneline._getAsyncs();var foundAsync=0;Oneline.asyncCallbacks[key]=fn;for(var i in currentAsyncs){if(currentAsyncs[i].key===key){ currentAsyncs[i].state=1;foundAsync=1;}}
...
return false;};Oneline.newSignature=function(){Oneline.signature=Oneline.uuid();};Oneline.joinOrNew=function(obj){var needsMerge=0;for(var i in Oneline.objects){if(Oneline.objects[i].type===obj.type){Oneline.objects[i]=obj;Oneline.objects[i].state=0;needsMerge=1;}}
...
return{ run:function(m){this.m=m||{};this.m.echo={};this.m.echo.limit=O.echo.options.limit;this.state=1;}}};Oneline.geolocation=Oneline.geo=function(options,ready){Oneline.geolocation.type="geolocation";Oneline.geolocation.options=options;if(!ready){var obj=clone(Oneline.geolocation(options,1));obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{ run:function(m)
{this.m=m||{};this.m.geo={};this.state=0;this.m.geo.every=O.geolocation.options.every;this.m.geo.range=O.geolocation.options.range;this.m.geo.limit=O.fetchOptional(O.geolocation.options.limit);var that=this;navigator.geolocation.getCurrentPosition(function(res){that.m.geo.lat=res.coords.longitude;that.m.geo.lng=res.coords.latitude;that.m.geo.range=O.geolocation.options.range;that.m.geo.limit=O.geolocation.options.limit;that.state=1;});}};};Oneline.event=function(options,ready){Oneline.event.options=options;Oneline.event.type="event";if(!ready){var obj=clone(Oneline.event(options,1));obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{ run:function(m)
{this.m=m||{};this.m.event=Oneline.event.options;this.state=1;}}};Oneline.generic=function(options,ready){Oneline.generic.options=options;if(!ready){var obj=clone(Oneline.generic(options,1));obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{ run:function(m)
{this.m=m||{};this.m.generic={};this.m.generic.type=Oneline.generic.options.type;this.m.generic.data=Oneline.generic.options.data;this.state=1;}}};Oneline.time=function(options,ready){Oneline.time.type="time";Oneline.time.options=options;if(!ready){var obj=clone(Oneline.time(options,1));obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{run:function(m)
{this.m=m||{};this.m.time={};this.m.time.start=Oneline.time.options.moment.start;this.m.time.end=Oneline.time.options.moment.end;this.state=1;}};};Oneline.random=function(options,ready){Oneline.random.type="random";Oneline.random.options=options;if(!ready){var obj=clone(Oneline.random(options,1));obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{run:function(m)
...
return;if(O.objects[c].state===1){m=collect(m,O.objects[c].m);c++;window['clear'+O.runner](O.oot);}}}}else{if(typeof O.objects[c].m!=='undefined'){m=collect(m,O.objects[c].m);}
c++;window['clear'+O.runner](O.oot);}},1);}}
O.ooot=window['set'+O.runner](function(){if(c===O.objects.length){m_.packet=m;if(Object.keys(m_.packet).length>0){t=Date.now();m_.packet.order=O.order;m_.packet.interop=O.interop;m_.asyncs=O._getActiveAsyncs();m_.uuid=O.uuid();m_.timestamp=t;m_.connection_uuid=O.connection_uuid;if(t>Oneline.after){O.socket.send(O.interop.stringify(m_));
}}
window['clear'+O.runner](O.ooot);}},1);},O.freq);}};};Oneline.sound=function(options,ready){Oneline.sound.options=options;if(!ready){var obj=clone(Oneline.sound(options,1));obj.signature=Oneline.signature;Oneline.joinOrNew(obj);}
return{run:function(m)
